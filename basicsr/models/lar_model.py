...
class LARModel(BaseModel):
    ...
    def init_training_settings(self): # 初始化和训练相关的 比如loss 设置optimizer 和 scheduler
        self.net_g.train()
    ...
    def setup_optimizers(self): # 具体设置optimizer，对params设置多组不同的optimizer
        train_opt = self.opt['train']
        optim_params = []
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img): # 单卡做validation
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger): # 如何控制打印validation的输出
    ...
    def save(self, epoch, current_iter): # 保存网络（.pth文件）以及训练状态（state文件）
        if hasattr(self, 'net_g_ema'):
